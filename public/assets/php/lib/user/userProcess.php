...
//    $ren=array();
    ...
    try{
        $pdo = connectDb('cooopshinren');
        $sql="select monthly_goods_id,goods_name,unit_price,detail_amount_per_one, required_quantity,category_name,color from monthly_goods natural join monthly natural join category where public_flag=0";
        $stmt=$pdo->prepare($sql);
